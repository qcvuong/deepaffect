...
class indiv:
	...
	def build_face_branch(inputs, chanDim=-1):
		...
		#model.load_weights("aa5EmotionBest.h5")
	...
	#receive the inputs from the other script (combined-MULT.py)
		...
		sceneBranch = indiv.build_scene_branch(inputs, finalAct=finalAct, chanDim=chanDim)
		objectBranch = indiv.build_object_branch(inputs, finalAct=finalAct, chanDim=chanDim)
		faceBranch = indiv.build_face_branch(inputs, chanDim=chanDim)
		...
		combinedInput = objectBranch.output

		valenceBranch = indiv.build_valence_branch(combinedInput, finalAct=finalAct)
		arousalBranch = indiv.build_arousal_branch(combinedInput, finalAct=finalAct)
